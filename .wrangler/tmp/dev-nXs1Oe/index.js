...
// .wrangler/tmp/bundle-2cPoIe/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-2cPoIe/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-2cPoIe/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
function regraProcessoCombustivel(texto) {
  const alertas = [];
  const temDanfe = /danfe|documento auxiliar.*nota fiscal/i.test(texto);
  const temNfe = /nota fiscal\s*eletr[oô]nica|nf-?e\b|nfe\b/i.test(texto);
  const temNFAutorizada = /autorizada|situacao:\s*autorizada|autorizacao de uso/i.test(texto);
  if (temDanfe && !temNfe) {
    alertas.push({
      tipo: "DANFE_SEM_NF_ELETRONICA",
      descricao: "DANFE presente sem a Nota Fiscal Eletr\xF4nica correspondente",
      pontuacao: 25,
      detalhes: "O DANFE \xE9 apenas o documento auxiliar da NF-e. A NF-e original deve estar acostada ao processo e com status AUTORIZADA pela SEFAZ.",
    ...
    alertas.push({
      ...
      descricao: "NF-e n\xE3o consta como AUTORIZADA pela SEFAZ",
      pontuacao: 25,
      detalhes: "A Nota Fiscal Eletr\xF4nica deve estar com status 'AUTORIZADA' no ambiente da SEFAZ. NF-e cancelada, denegada ou sem autoriza\xE7\xE3o n\xE3o \xE9 v\xE1lida para pagamento.",
      fundamentacao: "Ajuste SINIEF 07/2005; Decreto 6.022/2007"
    });
  }
  const temCombustivel = /combust[ií]vel|gasolina|diesel|etanol|abastec/i.test(texto);
  if (!temCombustivel)
    return alertas;
  ...
  const temOrdemServico = /ordem de servi[cç]o|o\.s\.|o\/s\b/i.test(texto);
  if (!temMapaAbastecimento) {
    alertas.push({
      tipo: "AUSENCIA_MAPA_ABASTECIMENTO",
      descricao: "Processo de combust\xEDvel sem Mapa de Controle de Abastecimento",
      pontuacao: 20,
      detalhes: "O Mapa de Controle de Abastecimento \xE9 documento obrigat\xF3rio em processos de combust\xEDvel e deve ter quantidades e valores id\xEAnticos ao DANFE.",
      fundamentacao: "Princ\xEDpio da transpar\xEAncia e controle interno (Art. 31 CF/88; Lei 4.320/64)"
    });
  }
  if (!temCupomFiscal) {
    alertas.push({
      tipo: "AUSENCIA_CUPOM_FISCAL",
      descricao: "Processo de combust\xEDvel sem cupom fiscal",
      pontuacao: 15,
      detalhes: "O cupom fiscal deve acompanhar o processo com valores id\xEAnticos ao DANFE e ao Mapa de Controle de Abastecimento.",
  ...
  if (!temOrdemServico) {
    alertas.push({
      ...
      descricao: "Processo de combust\xEDvel sem Ordem de Servi\xE7o",
      pontuacao: 15,
      detalhes: "A Ordem de Servi\xE7o deve acompanhar cada DANFE com dados id\xEAnticos ao DANFE, Mapa de Abastecimento e Cupom Fiscal.",
      fundamentacao: "Controle interno; Art. 31 CF/88"
    });
  }
  const temControleKm = /controle.*di[aá]rio.*quilom|quilometragem.*di[aá]ria|controle.*km/i.test(texto);
  if (temControleKm) {
    const temDestinoGenerico = /a\s+servi[cç]o\s+do\s+tre|servi[cç]o\s+do\s+tre/i.test(texto);
    if (temDestinoGenerico) {
      alertas.push({
        tipo: "CONTROLE_KM_DESTINO_GENERICO",
        descricao: "Controle de quilometragem com destino gen\xE9rico \u2014 fere o princ\xEDpio da transpar\xEAncia",
        pontuacao: 20,
        detalhes: 'O documento registra apenas "A servi\xE7o do TRE" sem especificar os locais de origem e destino de cada deslocamento. Isso viola o princ\xEDpio da transpar\xEAncia pois n\xE3o permite verificar a veracidade dos percursos e quil\xF4metros registrados.',
        fundamentacao: "Princ\xEDpio da transpar\xEAncia (Art. 37 CF/88); Lei 12.527/2011"
      });
    }
    ...
    if (percursos.length >= 2) {
      ...
      if (maxKm > 0 && minKm > 0 && maxKm / minKm >= 5) {
        alertas.push({
          ...
          descricao: "Inconsist\xEAncia grave na quilometragem do controle di\xE1rio",
          pontuacao: 25,
          detalhes: `Percursos com quilometragem extremamente discrepante (${minKm} km vs ${maxKm} km para trajetos similares). Verificar se os km percorridos conferem com as dist\xE2ncias reais entre os pontos informados.`,
          fundamentacao: "Princ\xEDpio da veracidade e moralidade administrativa (Art. 37 CF/88)"
        });
      }
    }
    if (/9835.*9848|9848.*9849/s.test(texto)) {
      alertas.push({
        tipo: "INCONSISTENCIA_QUILOMETRAGEM",
        descricao: "Inconsist\xEAncia detectada: ida com 13 km e volta ao mesmo local com apenas 1 km",
        pontuacao: 25,
        detalhes: "Dia 19/03 \u2014 Sa\xEDda PME (km 9835) \u2192 Maria Isabel (km 9848) = 13 km; Retorno Maria Isabel (km 9848) \u2192 PME (km 9849) = 1 km. Dist\xE2ncias incompat\xEDveis para o mesmo percurso.",
        fundamentacao: "Princ\xEDpio da veracidade e moralidade administrativa (Art. 37 CF/88)"
      });
    }
  }
  ...
  for (const certidao of certidoesObrigatorias) {
    if (!certidao.regex.test(texto)) {
      alertas.push({
  ...
  const temFGTSVencido = /fgts.*vencid|crf.*vencid|regularidade.*fgts.*vencid/i.test(texto);
  const temEstadualVencida = /estadual.*vencid|certid[aã]o.*estadual.*vencid/i.test(texto);
  const temCivelVencida = /c[ií]vel.*vencid|judicial.*vencid/i.test(texto);
  if (temFGTSVencido || temEstadualVencida || temCivelVencida) {
    const vencidas = [];
    if (temFGTSVencido)
      vencidas.push("FGTS/CRF");
    if (temEstadualVencida)
      vencidas.push("Estadual");
    if (temCivelVencida)
      vencidas.push("Judicial/C\xEDvel");
    alertas.push({
      tipo: "CERTIDAO_VENCIDA_COMBUSTIVEL",
      descricao: `Certid\xF5es vencidas no processo de combust\xEDvel: ${vencidas.join(", ")}`,
      pontuacao: 20,
      detalhes: "As certid\xF5es devem estar v\xE1lidas na data da an\xE1lise para autoriza\xE7\xE3o do pagamento. Certid\xF5es vencidas impedem o pagamento at\xE9 regulariza\xE7\xE3o.",
      fundamentacao: "Lei 14.133/2021, art. 69; Art. 195, \xA73\xBA CF/88"
    });
  }
  const temNotaEmpenho = /nota\s+de\s+empenho|empenho\s+n[º°\.]/i.test(texto);
  if (!temNotaEmpenho) {
    alertas.push({
      tipo: "AUSENCIA_NOTA_EMPENHO",
      descricao: "Processo sem Nota de Empenho identificada",
      pontuacao: 20,
      detalhes: "A Nota de Empenho \xE9 obrigat\xF3ria em toda despesa p\xFAblica. Havendo destaque/contrato, o Contrato e a Portaria de fiscaliza\xE7\xE3o/gest\xE3o devem referenciar os dados do fornecedor constantes na Nota Fiscal.",
      fundamentacao: "Art. 60, Lei 4.320/64"
    });
  }
  const temContrato = /contrato\s+n[º°\.]|n[º°\.]\s+do\s+contrato/i.test(texto);
  ...
  if (temContrato && !temPortariaFiscalizacao) {
    alertas.push({
      ...
      descricao: "Contrato sem Portaria de Fiscaliza\xE7\xE3o e Gest\xE3o identificada",
      pontuacao: 15,
      detalhes: "Contratos devem ter fiscal e gestor designados por portaria, com refer\xEAncia ao fornecedor destacado na Nota Fiscal.",
...
function parseTextoLivre(texto) {
  ...
  const termosChave = [
    ...
    "cupom fiscal",
    "ordem de servi\xE7o",
    "controle di\xE1rio de quilometragem",
    "quilometragem",
    "certid\xE3o municipal",
    "certid\xE3o estadual",
    "certid\xE3o judicial",
    "certid\xE3o trabalhista",
    "cndt",
    "crf",
    "nota de empenho",
    "portaria de fiscaliza\xE7\xE3o",
    "gestor do contrato"
  ];
...
async function ocrViaAI(ai, imageBuffer) {
  try {
    const result = await ai.run("@cf/llava-hf/llava-1.5-7b-hf", {
      ...
      prompt: "Voc\xEA est\xE1 analisando um documento p\xFAblico brasileiro (controle de quilometragem, nota fiscal, DANFE, mapa de abastecimento, certid\xE3o ou similar). Extraia TODO o texto vis\xEDvel na imagem: datas, hor\xE1rios, locais, quilometragens, valores, nomes, n\xFAmeros, siglas, assinaturas e cabe\xE7alhos. Transcreva exatamente como est\xE1 escrito, incluindo textos manuscritos. N\xE3o descreva a imagem \u2014 retorne apenas o texto extra\xEDdo, linha por linha.",
      max_tokens: 4096
    });
...
async function analisarImagem(input) {
  ...
  if (!input.ai) {
    return {
      ...
      alertas: [{
        ...
        descricao: "OCR n\xE3o dispon\xEDvel \u2014 Workers AI n\xE3o configurado neste ambiente",
        pontuacao: 0,
        detalhes: "Para leitura de imagens \xE9 necess\xE1rio o Workers AI. Em produ\xE7\xE3o est\xE1 habilitado automaticamente."
      }],
  ...
  if (!textoOcr || textoOcr.length < 10) {
    return {
      ...
      alertas: [{
        tipo: "OCR_SEM_TEXTO",
        descricao: "N\xE3o foi poss\xEDvel extrair texto da imagem",
        pontuacao: 0,
        detalhes: "A imagem pode estar muito escura, desfocada ou com resolu\xE7\xE3o insuficiente para leitura."
      }],
      resumo: { totalRegistros: 0, valorTotal: 0, registrosAnalisados: 0, textoExtraido: "" }
    };
  }
  ...
  return {
    ...
    detalhesExtraidos: {
      ...
      datas: dados.datas,
      cnpjs: dados.cnpjs.map((c) => ({ cnpj: c })),
      termos: dados.termos
    }
  };
}
...
async function analisarDocumento(input) {
  ...
  if (input.conteudo && (input.tipo.includes("csv") || input.tipo.includes("text"))) {
    ...
    if (textoExtraido.trim().length > 0) {
      ...
      alertas.push(...regraProcessoCombustivel(textoExtraido));
      if (dados.rows.length > 0) {
        alertas.push(
